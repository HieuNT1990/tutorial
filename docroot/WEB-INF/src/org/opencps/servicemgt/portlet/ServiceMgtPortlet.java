...
public class ServiceMgtPortlet extends MVCPortlet {
	 ...
	 * @throws IOException
	 */
	public void deleteDomain(ActionRequest actionRequest,
	 ...
	 * @throws IOException
	 */
	public void updateDomain(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException {
		long dictItemId = ParamUtil.getLong(actionRequest,
				DictItemDisplayTerms.DICTITEM_ID);
		long parentItemId = ParamUtil.getLong(actionRequest,
				DictItemDisplayTerms.PARENTITEM_ID);
		String itemName = ParamUtil.getString(actionRequest,
		...
		try {
			...
			DataMamagementPortlet.validatetDictItem(dictItemId, itemName,
					itemCode, serviceContext);
			if (dictItemId == 0) {
				...
				if (Validator.isNotNull(isAddchirld)) {
					...
					DictItemLocalServiceUtil.updateDictItem(dictItemId,
							dictCollection.getDictCollectionId(), 0, itemCode,
							itemNameMap, parentItemId, serviceContext);
					SessionMessages.add(actionRequest,
	 ...
	 */
	public ServiceInfo updateService(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException {

		ServiceDisplayTerms displayTerms = new ServiceDisplayTerms(
				actionRequest);
		...
		try {
			...
			if (serviceInfoId <= 0) {
				...
				ServiceInfoLocalServiceUtil.addService(
						displayTerms.getServiceNo(),
						displayTerms.getServiceName(),
						...
						displayTerms.setFileTemplateIds(actionRequest),
						serviceContext);
				...
				// Update ServiceInfo
				ServiceInfoLocalServiceUtil.updateService(serviceInfoId,
						displayTerms.getServiceNo(),
						displayTerms.getServiceName(),
						displayTerms.getFullName(),
						displayTerms.getServiceProcess(),
	 ...
	 * @throws IOException
	 */
	public void updateTempalteFile(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException {
			...
			if (templateFileId <= 0) {
				...
				if (Validator.isNotNull(file)) {
					...
					TemplateFileLocalServiceUtil.addServiceTemplateFile(
							file.getFileEntryId(), fileNo, fileName,
							serviceContext);
					SessionMessages.add(actionRequest,
	 ...
	 * @throws IOException
	 */
	public void chooseServiceInfo(ActionRequest actionRequest,
			ActionResponse actionResponse) throws PortalException,
			SystemException, IOException {
		long templatefileId = ParamUtil
				.getLong(actionRequest, "templateFileId");
		long[] serviceInfoIds = ParamUtil
				.getLongValues(actionRequest, "rowIds");
		String backURL = ParamUtil.getString(actionRequest, "backURL");
		TemplateFileLocalServiceUtil.addChooseServceInfo(templatefileId,
				serviceInfoIds);
		if (Validator.isNotNull(backURL)) {
			actionResponse.sendRedirect(backURL);
		}
	 ...
	 * @throws IOException
	 */
	public void deteleRelaSeInfoAndTempFile(ActionRequest actionRequest,
			ActionResponse actionResponse) throws PortalException,
			SystemException, IOException {
		long templatefileId = ParamUtil
				.getLong(actionRequest, "templateFileId");
		...
		String backURL = ParamUtil.getString(actionRequest, "backURL");
		ServiceFileTemplateLocalServiceUtil.deleteServiceFile(serviceInfoId,
				templatefileId);
		if (Validator.isNotNull(backURL)) {
			actionResponse.sendRedirect(backURL);
		}
	...
	public void deleteTemplate(ActionRequest actionRequest,
			ActionResponse actionResponse) throws PortalException,
			SystemException, IOException {
	...
	public void deleteService(ActionRequest actionRequest,
			ActionResponse actionResponse) throws PortalException,
			SystemException, IOException {
	...
	public void render(RenderRequest renderRequest,
		...
		long templatefileId = ParamUtil
				.getLong(renderRequest, "templateFileId");
		...
		try {
			...
			templateFile = TemplateFileLocalServiceUtil
					.fetchTemplateFile(templatefileId);

			serviceInfo = ServiceInfoLocalServiceUtil
					.fetchServiceInfo(serviceinfoId);
		} catch (Exception e) {
			_log.info(e);
		}
		...
		renderRequest
				.setAttribute(WebKeys.SERVICE_TEMPLATE_ENTRY, templateFile);
	...
	protected FileEntry updateFileEntry(ActionRequest actionRequest,
		...
		try {
			...
			fileEntry = DLAppServiceUtil.addFileEntry(repositoryId, folderId,
					sourceFileName, contentType, sourceFileName, description,
					changeLog, inputStream, size, serviceContext);

			AssetPublisherUtil
					.addAndStoreSelection(actionRequest,
	...
	private static final Log _log = LogFactoryUtil
			.getLog(ServiceMgtPortlet.class);
}
